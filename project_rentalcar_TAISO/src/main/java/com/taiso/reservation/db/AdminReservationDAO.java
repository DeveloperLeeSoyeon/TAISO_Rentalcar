...
public class AdminReservationDAO {
   ...
   private Connection getConnection() throws Exception {
      ...
      // 예약 전체 개수 확인 - getReservationCount
         ...
         // 예약 전체 개수 확인 - getReservationCount(String mem_id)
      ...
      // 예약 전체 개수 확인 - getReservationCount
         ...
         */
         ...
         // 예약 전체 개수 확인 - getsReservationCount(String mem_id)
         public int getsReservationCount(String type, String keyword) {
            int cnt = 0;
            ...
            try {
               String colName="";
               switch(type) {
               case "1" : colName="mem_id";
                        break;
               case "2" : colName="rez_uqNum";
                         break;         
               }      
               //1.2. 디비연결
               con = getConnection();
               // 3. sql
               sql = "select count(*) from rez_reservation where "+colName+ " like ?";
               pstmt = con.prepareStatement(sql);
         ...
         // 검색된 회원의 예약 정보 조회
         public List getSearchList(String type, String keyword, int startRow,int pageSize) {
             ...
             try {
               ...
               case "2" : colName="rez_uqNum";
                ...
                con = getConnection();
                // 3. sql 작성 &  pstmt객체
                sql = "select * from rez_reservation where "+colName+ " like ? order by rez_uqNum desc limit ?,?";
