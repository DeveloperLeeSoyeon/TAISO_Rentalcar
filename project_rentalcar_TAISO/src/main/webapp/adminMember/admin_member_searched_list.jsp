...
//       return false;
//    }
	 if(document.fr.type.selectedIndex==0){
	      alert("목록 선택하세요");
	      document.fr.type.focus();
	      return false;
	   }
			...
			<div class="container">
				<div
					...
					<div class="col-md-9 ftco-animate pb-5">
						...
						<h1 class="mb-3 bread">관리자 회원 조회</h1>
					</div>
				</div>
			</div>
		</section>
								...
								<div class="row">
									<div class="board_wrap">
										<div class="board_title">
											...
											<div border="1" width="90%" align="right"></div>
										</div>
										<div class="table table-hove">
											<div class="board_list">
												...
												<strong>회원 검색</strong>
													...
													<!--  검색 폼 -->	
													<div class="m_search" border="1" width="90%">
														<div align="right">
															<form name="fr" action="./SearchMemberList.mb" onsubmit="return fun1()">
																<select name="type" id="type" > 
																	<option value="">::검색항목::</option>
																	<option value="1">회원번호</option>
																	<option value="2">회원아이디</option>
																	<option value="3">회원이름</option>
																</select>
												...
												<form name="fr">
													<table class="table table-hover" align="center">
														...
														<tbody>
															<c:forEach var="mDTO" items="${searchMemberList }">
																<tr colspan="1" align="center">
																	...
																	<td><input type="button" value="회원상세정보"
																		class="btn btn-primary py-0.5 px-1"
																		onclick="location.href='./AdminMemberInfo.mb?mem_num=${mDTO.mem_num}&pageNum=${pageNum }';">
																	</td>
																</tr>
															</c:forEach>
														</tbody>
											...
											<div class="n_page" align="center">
												<c:if test="${requestScope.cnt != 0}">
													...
													<c:if test="${startPage > pageBlock }">
														<a
															href="./AdminMemberList.mb?pageNum=${startPage-pageBlock }&type=${type }&keyword=${keyword }"
															class="btn btn-outline-primary"></a>
													</c:if>
													<!-- 페이지 번호(1,2,3....) -->
													<c:forEach var="i" begin="${startPage }" end="${endPage }"
														step="1">
														<a href="./AdminMemberList.mb?pageNum=${i }&type=${type }&keyword=${keyword }"
										...
										<div class="table table-hove">
														...
														<c:forEach var="mDTO" items="${blackListAll }">
															<tr colspan="1" align="center">
																...
																<td><input type="button" value="상세정보"
																	...
																	class="btn btn-primary py-0.5 px-1"
																	onclick="location.href='./AdminMemberDelete.mb?mem_id=${mDTO.mem_id }&pageNum=${pageNum }';">
																</td>
															</tr>
														</c:forEach>
													</tbody>
											...
											<div class="n_page" align="center">
												<c:if test="${requestScope.cnt != 0}">
													...
													<c:if test="${startPage > pageBlock }">
														<a
															href="./AdminMemberBlackList.mb?pageNum=${startPage-pageBlock }"
															class="btn btn-outline-primary"><<</a>
													</c:if>
													<!-- 페이지 번호(1,2,3....) -->
													<c:forEach var="i" begin="${startPage }" end="${endPage }"
														step="1">
														<a href="./AdminMemberBlackList.mb?pageNum=${i }"
															class="btn btn-outline-primary">${i }</a>
													</c:forEach>
													<!-- 다음 -->
													<c:if test="${endPage < pageCount }">
